...
in_tok=$(jq_get '.context_window.total_input_tokens')
out_tok=$(jq_get '.context_window.total_output_tokens')
tok_info=""
if [ -n "$in_tok" ]; then
  tok_info="${DIM}in${R} ${BOLD}$(fmt_tokens "$in_tok")${R}"
fi
if [ -n "$out_tok" ]; then
  [ -n "$tok_info" ] && tok_info="${tok_info}${sep}"
  tok_info="${tok_info}${DIM}out${R} ${BOLD}$(fmt_tokens "$out_tok")${R}"
fi

output="$line1"
if [ -n "$line2" ] || [ -n "$tok_info" ]; then
  line2_parts=""
  if [ -n "$tok_info" ]; then
    line2_parts="$tok_info"
  fi
  if [ -n "$line2" ]; then
    [ -n "$line2_parts" ] && line2_parts="${line2_parts}${sep}"
    line2_parts="${line2_parts}${line2}"
  fi
  output="${output}"$'\n'"${line2_parts}"
fi
